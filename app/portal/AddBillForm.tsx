...
export default function AddBillForm({
  ...
  const selected = useMemo(
    () => billTypes.find((t) => t.name === type) ?? null,
    [billTypes, type],
  );
  const fee = selected?.processingFee ?? 0;
          ...
          <form action={formAction}>
            ...
            {selected?.ownerName && (
              <p className="mt-4 text-sm text-ink-muted">
                {selected.emoji} {selected.name} is on{" "}
                <strong className="text-ink">{selected.ownerName}</strong>&rsquo;s account —
                everyone else pays her their share
                {base > 0 ? <span className="figure"> (${cost.toFixed(2)} each)</span> : null}.
              </p>
            )}
