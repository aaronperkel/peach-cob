...
- `people` (`id`, `name`, `email` = also the login identity (unique), `is_admin`, `splits_bills` — 0 for sign-in-only accounts like Aaron the maintainer: they can run the portal but are excluded from split math, debts, checkbox lists, and new-bill emails via `getSplitters()`; `welcomed_at` — NULL until the person finishes the `/welcome` tour, which the login actions redirect first-timers through)
...
- `app/portal/` — portal in two tabs (`PortalTabs`): `/portal` = bills (still-owed strip, add-bill disclosure — the form asks only type/amount/due/PDF; `bill_date` is stamped automatically with the NY posting date — pay-back checkboxes that exclude the bill's owner, per-bill reminders), `/portal/household` = residents + utilities (name/emoji/fee/owner) + reminder schedule. All mutations are server actions; flash messages travel as `?ok=`/`?err=` query params, and `done()`/`fail()` in `actions.ts` take the destination path so household actions land back on their tab
- `app/welcome/` — first-login welcome tour: five auto-advancing animated vignettes (client `Tour.tsx`, `.tour-*` keyframes in `globals.css`, no autoplay under `prefers-reduced-motion`) walking through owner → post → ledger → check-off → reminders. Both login actions redirect here while `people.welcomed_at` is NULL; finishing or skipping stamps it (`finishWelcome`) and lands on `/`. Revisitable anytime via the footer's "how this works" link
