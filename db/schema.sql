-- db/schema.sql
-- Schema for Peach Cob (TiDB Cloud Serverless, MySQL dialect).
--
...
-- pays the provider, so when a bill posts the OTHER residents owe her their
-- shares (owner_id NULL falls back to everyone owing, nobody collecting).
CREATE TABLE bill_types (
    id             INT UNSIGNED  NOT NULL AUTO_INCREMENT PRIMARY KEY,
    name           VARCHAR(50)   NOT NULL UNIQUE, -- 'Wifi', 'Electric', 'Gas', 'Water', ...
    emoji          VARCHAR(16)   NOT NULL,
    processing_fee DECIMAL(10,2) NOT NULL DEFAULT 0,
    owner_id       INT UNSIGNED  DEFAULT NULL     -- references people.id
) DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE bills (
    ...
    pdf_path        VARCHAR(255)  DEFAULT NULL,          -- '{year}/{type}/{file}.pdf' in Vercel Blob
    added_by_id     INT UNSIGNED  DEFAULT NULL,          -- references people.id; who posted it
...
-- A row here means the person still owes their share of the bill (to the bill
-- type's owner). The owner never gets a row — her share is the part she
-- already fronted to the provider. Rows are deleted as people pay; when none
-- remain the bill flips to 'paid' (see updateOwes in app/portal/actions.ts).
